 -function () {
   ...
   function drawPart(data, id, p) {
     ...
     mainbar.append("text").attr("class", "barlabel")
       ...
       .text(function (d, i) { return data.keys[p][i] + " " + data.mainBars[p][i].value + " (" + Math.round(data.mainBars[p][i].percent * 100) + ")"; })
   ...
   function drawHeader(data, header, id) {
     ...
     [0, 1].forEach(function (d) {
       ...
       h.append("text").text(header[d] + " 人次 (%)").attr("x", (me.options.labelColumn[d] - 5)).attr("y", -5).attr('text-anchor', 'start');
...
function transitionHeader(data, header, id) {
  ...
  [0, 1].forEach(function (d) {
    ...
    h.select("text").text(header[d] + " 人次 (%)").attr("x", (me.options.labelColumn[d] - 5)).attr("y", -5).attr('text-anchor', 'start');
    var sum = d3.sum(data.mainBars[d].map(function(d){return d.value;}));
    //h.append("text").text(sum + "(100%)").attr("x", (me.options.valueColumn[d] - 10)).attr("y", -5);

    h.select("line").attr("x1", me.options.labelColumn[d] - 10).attr("y1", -2).attr("x2", me.options.barpercentColumn[d] + 10).attr("y2", -2)
      .style("stroke", "black").style("stroke-width", "1").style("shape-rendering", "crispEdges");
  });
}
