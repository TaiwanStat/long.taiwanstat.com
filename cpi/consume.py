...
def plot_consume(i):
	...
	for dataset in root[1][flr: ceil]:
		...
		for detail in raw:
			...
			obs_val.append(float(detail.get('OBS_VALUE')))
...
while i < 4:
	df = plot_consume(i)
	if(i == 0):
		axes[i].set_title(u'台灣全體家庭：所得層級別消費者物價指數')
	elif(i == 1):
		axes[i].set_title(u'台灣最低所得 20 %：所得層級別消費者物價指數')
	elif(i == 2):
		axes[i].set_title(u'台灣中間所得 60 %：所得層級別消費者物價指數')
	elif(i == 3):
		axes[i].set_title(u'台灣最高所得 20 %：所得層級別消費者物價指數')
